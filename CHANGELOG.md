...
### Changed
- ตัวติดตั้ง Windows ใหม่: custom splash ธีมครูดี คมชัดทุก DPI (งานจาก v1.2.3)

### Fixed
- แก้ release v1.2.3 ที่เลขเวอร์ชันในแอปยังค้างเป็น 1.2.2 ทำให้ auto-update มองไม่เห็นเวอร์ชันใหม่ — bump เวอร์ชันจริงเป็น 1.2.4
...
### Added
- แบนเนอร์เตือนบนหน้าสแกนเมื่อเครื่องรันเวอร์ชันสูงกว่าที่เซิร์ฟเวอร์อนุญาต (`version_status: ahead` จาก heartbeat) — เตือนแต่ยังใช้งานได้ปกติ

### Fixed
- heartbeat รายงานเวอร์ชันจริงของแอปที่รันอยู่ (`app.getVersion()`) แทนค่าที่ค้างใน config ตั้งแต่ตอนติดตั้งครั้งแรก — เซิร์ฟเวอร์ตรวจจับการแอบติดตั้งเวอร์ชันเองได้แล้ว
- refresh ค่า `app_version` ใน config ทุกครั้งที่เปิดแอป

## [1.2.0] - 2026-07-21

### Added
- ปุ่ม "ตรวจหาอัปเดต" และ "ติดตั้งและรีสตาร์ททันที" ในแท็บตั้งค่า (Admin) — ติดตั้งเวอร์ชันใหม่ได้ทันทีโดยไม่ต้องรอหน้าต่างเวลาที่ตั้งไว้ (ยังเคารพ `target_version` ของเซิร์ฟเวอร์)
- แสดงข้อความแจ้งเตือนเมื่อตรวจหาอัปเดตไม่สำเร็จ (ติดต่อเซิร์ฟเวอร์ไม่ได้) — กดตรวจใหม่เองได้ภายหลัง

### Changed
- ตัวติดตั้ง Windows (NSIS) เปลี่ยนเป็นแบบคลิกเดียว (oneClick, perMachine) — เหมาะกับ auto-update แบบเงียบบนเครื่องคีออสก์ พร้อมสร้างไฟล์ portable เพิ่ม

## [1.1.1] - 2026-07-21

### Changed
- ทดสอบระบบอัปเดตอัตโนมัติแบบให้เซิร์ฟเวอร์ครูดีเป็นตัวคุมเวอร์ชัน — release นี้ใช้ยืนยันว่าเครื่องจะอัปเดตเป็น 1.1.1 ก็ต่อเมื่อเซิร์ฟเวอร์ตั้ง `target_version` อนุญาตแล้วเท่านั้น
...
- เซิร์ฟเวอร์คุมเวอร์ชันที่อนุญาตให้ติดตั้งได้ต่อโรงเรียน (`target_version`) — kiosk จะไม่โหลด/ติดตั้งเวอร์ชันที่เกินจากนี้
...
### Added
- ปุ่ม "โหมดผู้ดูแล" บนหน้า Kiosk (มุมบนขวา) — เข้าหน้า Admin ได้โดยไม่ต้องใช้ปุ่มลัด Ctrl+Shift+A (ปุ่มลัดยังใช้ได้)
- ปุ่ม "ปิดโปรแกรม" บนหน้า Kiosk — ต้องใส่ PIN ผู้ดูแลก่อนปิดโปรแกรม เพื่อกันนักเรียนกดปิดเล่น

## [1.0.9] - 2026-07-17

### Added
- แท็บ "สรุปวันนี้" ในหน้า Admin — นักเรียนทั้งหมด/มาแล้ว/มาสาย/กลับแล้ว/ยังไม่มา แยกรายห้อง พร้อมเกณฑ์มาสายตั้งค่าได้ (`late_after`, ค่าเริ่มต้น 08:30)
...
### Changed
- โหมด both: ตัดสินเข้า/ออกแบบอิงเวลาล้วน — แตะหลังเวลาตัด (ค่าเริ่มต้น 10:00) ถือว่า "กลับบ้าน"
...
- ไฟล์ Linux (AppImage/deb) ไม่มีเลข version ในชื่อไฟล์ — startup application ชี้ path เดิมได้ทุกเวอร์ชัน
...
### Added
- ตั้งค่า "กันแตะซ้ำ (นาที)" ได้จากหน้า Admin — ไม่ fix ที่ 30 นาทีอีกต่อไป
- ตั้งค่า "นับเป็น ออก ตั้งแต่กี่โมง" (exit_after_hour, ค่าเริ่มต้น 10:00) — แตะบัตรหลังเวลานี้นับเป็น "ออกโรงเรียน" อัตโนมัติ
- แตะซ้ำชนิดเดิมใน cooldown ถูกบล็อก แต่ถ้าเปลี่ยนชนิด (เข้า→ออก) บันทึกได้ทันที

### Fixed
- หน้า "ผู้ดูแลเครื่อง" tab "ตั้งค่า" scroll ไม่ได้ กดปุ่มบันทึกไม่ถึง — จำกัดความสูงหน้า admin ให้ scroll ภายในตัวเอง
- แตะบัตรครั้งที่ 2 ยังขึ้น "เข้าโรงเรียน" — เปลี่ยน logic เข้า/ออกเป็นแบบอิงเวลา (time-based cutoff)
