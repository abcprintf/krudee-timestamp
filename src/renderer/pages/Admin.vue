...
async function saveSettings(): Promise<void> {
  ...
  loading.value = true
  try { config.value = await window.krudee.admin.updateSettings({ base_url: config.value.base_url, device_name: config.value.device_name ?? '', role: config.value.role, scan_cooldown_minutes: config.value.scan_cooldown_minutes, tts_enabled: config.value.tts_enabled, auto_start: config.value.auto_start }) }
