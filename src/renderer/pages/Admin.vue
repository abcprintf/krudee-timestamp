...
import type { AppConfigView, ClockSkew, DailySummary, HistoryRow, RfidCard, StudentView, UnknownUidRow, UpdateStatus } from '../env'
...
const changelog: { version: string; date: string; added?: string[]; fixed?: string[] }[] = [
  ...
  },
  {
    ...
    date: '21 กรกฎาคม 2569',
    added: [
      'ปุ่ม "ตรวจหาอัปเดต" และ "ติดตั้งและรีสตาร์ททันที" ในแท็บตั้งค่า — ติดตั้งเวอร์ชันใหม่ได้ทันทีไม่ต้องรอหน้าต่างเวลา (ยังเคารพเวอร์ชันที่เซิร์ฟเวอร์อนุญาต)',
      'แจ้งเตือนเมื่อตรวจหาอัปเดตไม่สำเร็จ (ติดต่อเซิร์ฟเวอร์ไม่ได้) — กดตรวจใหม่เองได้ภายหลัง',
      'ตัวติดตั้ง Windows เปลี่ยนเป็นแบบคลิกเดียว (NSIS oneClick) เหมาะกับการอัปเดตอัตโนมัติบนเครื่องคีออสก์',
    ],
  },
  {
    version: '1.1.1',
  ...
  },
  {
    ...
    added: [
      'อัปเดตแอปอัตโนมัติแบบไร้คนดูแลในหน้าต่างเวลาที่ตั้งได้ (ค่าเริ่มต้น 12:00-13:00) — ติดตั้งเฉพาะตอนไม่มีการแตะบัตรอย่างน้อย 2 นาที',
      'เซิร์ฟเวอร์คุมเวอร์ชันที่อนุญาตให้ติดตั้งได้ต่อโรงเรียน (target_version)',
      'ตั้งค่าใหม่ในหน้า Admin: หน้าต่างเวลาติดตั้งอัปเดต, เปิด/ปิดติดตั้งอัตโนมัติ, แสดงเวอร์ชันปัจจุบัน/เวอร์ชันที่เซิร์ฟเวอร์อนุญาต',
      'ไฟล์ติดตั้ง Linux เหลือเฉพาะ AppImage (ตัด .deb) — ให้ auto-update ทำงานได้จริง',
    ],
  },
  {
    version: '1.0.10',
...
const updateStatusText = computed(() => {
  ...
  if (!s) return 'กำลังโหลด...'
  if (!s.enabled) return 'ใช้ได้เฉพาะเวอร์ชันที่ติดตั้งแล้ว (ไม่รองรับในโหมดพัฒนา)'
  if (s.downloaded) return `พร้อมติดตั้ง v${s.downloaded} — กดปุ่มเพื่อรีสตาร์ททันที ไม่ต้องรอหน้าต่างเวลา`
  if (s.available) return `กำลังดาวน์โหลด v${s.available}... ${s.percent}%`
  return `เวอร์ชันปัจจุบัน v${s.current} · เซิร์ฟเวอร์อนุญาตถึง ${s.target ?? '—'} — เป็นเวอร์ชันล่าสุดแล้ว`
})
...
async function saveSettings(): Promise<void> {
  ...
  try {
    const payload: Record<string, string> = {
      ...
      update_window_start: config.value.update_window_start ?? '12:00', update_window_end: config.value.update_window_end ?? '13:00',
      update_auto_install: config.value.update_auto_install ?? 'true',
    }
...
async function refreshUpdateStatus(): Promise<void> {
  try { updateStatus.value = await window.krudee.updater.status(); updateError.value = '' }
  catch { updateError.value = 'อ่านสถานะอัปเดตไม่สำเร็จ — กดตรวจหาอัปเดตเพื่อลองใหม่' }
}

async function checkUpdate(): Promise<void> {
  updateBusy.value = true
  updateError.value = ''
  try { updateStatus.value = await window.krudee.updater.check() }
  catch { updateError.value = 'ตรวจสอบอัปเดตไม่สำเร็จ — อาจติดต่อเซิร์ฟเวอร์ไม่ได้ กรุณากดตรวจหาอัปเดตอีกครั้งภายหลัง' }
  finally { updateBusy.value = false }
}

async function installUpdate(): Promise<void> {
  updateBusy.value = true
  try { await window.krudee.updater.install() }
  catch { updateBusy.value = false }
}
