...
import type { AppConfigView, HistoryRow, RfidCard, StudentView, UnknownUidRow } from '../env'
...
const pin = ref('')
const error = ref('')
const tab = ref<'students' | 'cards' | 'settings' | 'history' | 'changelog'>('students')
...
async function unlock(): Promise<void> {
  error.value = ''
  unlocked.value = await window.krudee.admin.verifyPin(pin.value)
  if (!unlocked.value) { error.value = 'PIN ไม่ถูกต้อง'; return }
  await loadAll()
}
...
async function saveSettings(): Promise<void> {
  ...
  loading.value = true
  try { config.value = await window.krudee.admin.updateSettings({ base_url: config.value.base_url, device_name: config.value.device_name ?? '', role: config.value.role, exit_after_hour: config.value.exit_after_hour, scan_cooldown_minutes: config.value.scan_cooldown_minutes, tts_enabled: config.value.tts_enabled, auto_start: config.value.auto_start }) }
